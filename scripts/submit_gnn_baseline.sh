...
#SBATCH -o /ibmgpfs/cuizaixu_lab/xuhaoshu/projects/sc_connectome_trajectories/outputs/logs/gnn_baseline/%A_%a.out
#SBATCH -e /ibmgpfs/cuizaixu_lab/xuhaoshu/projects/sc_connectome_trajectories/outputs/logs/gnn_baseline/%A_%a.err

set -euo pipefail

module load singularity

mkdir -p /ibmgpfs/cuizaixu_lab/xuhaoshu/projects/sc_connectome_trajectories/outputs/logs/gnn_baseline
...
render_paths_output="$("$PYTHON_HOST_BIN" -m scripts.render_paths \
  --set CONTAINER=local.containers.torch_gnn \
        ...
        RESULTS_DIR=local.outputs.gnn_baseline \
  --resolve)" || {
...
RUN_BASE="${RUN_BASE:-gnn_baseline_${TS}_job${ARRAY_JOB_ID}}"
...
singularity exec --nv \
  ...
  --bind /GPFS:/GPFS \
  "$CONTAINER" \
  "$PYTHON_BIN" -m scripts.train_gnn_baseline \
