...
- 为更清晰地展示“短间隔不劣于 identity、长间隔学到变化”，建议按测试对的 `dt_months` 进行分位数分层：
  - Short：`dt_months ≤ P33(dt_months)`
  - Long：`dt_months ≥ P67(dt_months)`
  - （可选）Mid：其余样本
- 报告时务必写出 `P33/P67` 对应的具体月份阈值与各桶样本量（例如 “Short（≤4.8 months, n=xxx），Long（≥13.2 months, n=xxx）”）。
