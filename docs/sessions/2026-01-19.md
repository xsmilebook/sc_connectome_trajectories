...
## 新增：density 收敛性实验（10 个任务，待提交/运行）

为解决 “density 一开就 early stop，导致还没收敛” 的问题，本次补充了：

1) 可选 early stopping 监控口径（训练期在验证集计算 SC 指标）
   - `early_stop_metric` 支持：`val_loss`（默认）、`val_sc_log_pearson_sparse/topk`、`val_sc_log_mse`、`monitor_mse_plus_density`。
   - `val_sc_eval_every` 控制每 N 个 epoch 计算一次验证集 SC 指标（密度实验推荐设为 1）。

2) 一组 10 个提交脚本（fold0）
   - `bash scripts/submit_clg_ode_density_10exp_batch.sh`
   - 8 组网格：warmup（2/5）× `lambda_density`（0.002/0.005/0.01/0.02），更长训练（100 epoch）+ 更大 patience（25）。
   - 2 组两阶段训练：Phase-1 先学主任务，Phase-2 学习率降低并打开 density/zero；Phase-2 分别用 `val_sc_log_pearson_sparse` 与 `monitor_mse_plus_density` 作为监控。
