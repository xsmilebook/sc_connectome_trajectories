...
## 输出：fold0 技术报告（扩展拓扑指标版）

- 汇总并写入报告：`docs/reports/clg_ode_fold0_test_eval_topology_20260121.md`
  - 覆盖 B1/B1′/A1/B2/C2/B3/D2/D2′（fold0），并使用 `test_sc_metrics_ext.json` 统一补齐 degree/strength 指标。

## 补充：复现材料清单与测试样例打包

- 新增 `requirements.txt`：对齐容器 `scripts/containers/torch_gnn.def` 的依赖清单（用于复现材料/容器构建参考）。
- 新增 `docs/reproducibility.md`：整理依赖/配置、复现实验命令、数据与 checkpoints 定位方式。
- 新增 `scripts/prepare_test_sample_subject.sh`：由用户按需执行，将一个被试的处理后数据复制到 `outputs/results/` 下作为测试样例包（不修改原始 `data/`）。
