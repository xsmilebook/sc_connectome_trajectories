...
- 修复 test 指标聚合键错误（新增相关性字段后不再触发 KeyError），并增加 RUN_DATE/RUN_TIME 控制统一 run 目录名。
- 将默认拓扑损失权重下调为 `lambda_topo=1e-3`，避免 loss 量级失衡。
- 增加每个 fold 的长度分布日志与 `vel/acc` 计数，并为单卡 `torchrun` 自动挑选可用端口。
- 文档补充：更新集群 GPU 使用说明与训练工作流，记录端口冲突与日志目录等常见问题处理方式。
