...
- 入口规范化：新增根目录 `scripts/` 作为统一执行入口（`python -m scripts.<entry>`），并将 `src/` 调整为可复用模块包（可被 `scripts/` 直接 import）。
- 路径配置化：新增 `configs/paths.yaml`，保留 FreeSurfer 的 `/GPFS/.../ABCD/...` 绝对路径（不迁移），其余默认路径改为相对的 `data/*` 与 `outputs/*`，并移除 `src/` 下的运行时 sublist 文件。
